...
for i in range(100000):
    ...
    coeff=ab_solution[0]
    if i%100==0:
        print(coeff, np.mean(solution), np.sum(Ta-(p_sol@solution)*calc_spec_profile(normalized_f, coeff))**2)


plt.plot(angles_sol, solution)
plt.plot(angles_sim, sky_model)
plt.show()

plt.plot(freqs, (Ta-(p_sol@solution)*calc_spec_profile(normalized_f, coeff))*1e3)
plt.show()